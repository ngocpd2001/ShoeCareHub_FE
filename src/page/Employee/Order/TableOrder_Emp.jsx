import React, { forwardRef, useEffect, useImperativeHandle, useState } from "react";
  ...
  import { getOrdersByBranchId, getBranchByBranchId } from "../../../api/branch";
...
export const TableOrder_Emp = forwardRef((props, ref) => {
  ...
  const modalEdit = useModalState();
        ...
        render: (_, record) => (
          <div className="flex items-center flex-col">
            <ComMenuButonTable
              ...
              showModalEdit={() => {
                const orderId = record.id;
                navigate(`/employee/order/update/${orderId}`);
              }}
    ...
    const getBranchId = () => {
      ...
      return JSON.parse(userData)?.branchId;
    };
    ...
    const reloadData = async () => {
      ...
      try {
        ...
        // Gọi cả 2 API
        const [branchResponse, orderResponse] = await Promise.all([
          getBranchByBranchId(branchId),
          getOrdersByBranchId(branchId)
        ]);
  
        // Đảm bảo orderResponse là một mảng
        if (Array.isArray(orderResponse)) {
          ...
          setData([]); // Set empty array if response is invalid
          console.error("Invalid order response format:", orderResponse);
        }
  
        // Lấy data từ branchResponse
        const branchData = branchResponse;
        
        setStatistics({
          pendingAmount: branchData.pendingAmount || 0,
          processingAmount: branchData.processingAmount || 0,
          finishedAmount: branchData.finishedAmount || 0,
          canceledAmount: branchData.canceledAmount || 0
        });
